...
function connectWebSocket(subId, wsUrl) {
  ...
  try {
    ...
    socket.onerror = function(error) {
      ...
      isConnected = false;
      connectionInProgress = false;
      notifyClients({ type: 'WEBSOCKET_ERROR', data: { error: error.message || 'WebSocket connection failed' } });
    };
    
  } catch (error) {
    console.error('[SW] Error creating WebSocket:', error);
    connectionInProgress = false;
    notifyClients({ type: 'WEBSOCKET_ERROR', data: { error: error.message || 'Failed to create WebSocket' } });
  }
}
